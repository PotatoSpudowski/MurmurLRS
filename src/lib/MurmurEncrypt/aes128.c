/*
 * aes128.c — AES-128 ECB encrypt (FIPS 197)
 *
 * Portable C, no dynamic allocation. Suitable for ESP32, STM32,
 * and desktop. ~1 KB code + 256-byte S-box.
 */
#include "aes128.h"
#include <string.h>
...
void aes128_encrypt(const uint8_t key[16], const uint8_t in[16], uint8_t out[16])
{
    ...
    memcpy(out, in, 16);
    add_round_key(out, rk);

    for (int r = 1; r <= 9; r++) {
        sub_bytes(out);
        shift_rows(out);
        mix_columns(out);
        add_round_key(out, rk + r * 16);
    }

    /* Round 10: no MixColumns */
    sub_bytes(out);
    shift_rows(out);
    add_round_key(out, rk + 160);
}
